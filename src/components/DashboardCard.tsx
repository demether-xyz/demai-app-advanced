...
const DashboardCard: React.FC<DashboardCardProps> = ({ id, title, icon, color, aiPriority, category, content, isExpanded, onExpand }) => {
  if (isExpanded) return null

  return (
    <div
      onClick={() => onExpand(id)}
      className="group relative w-full cursor-pointer rounded-xl border border-slate-700/40 bg-slate-900/60 backdrop-blur-md transition-all duration-200 hover:bg-slate-800/60"
    >
      {/* Card Content */}
      <div className="p-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            {icon && (
              <div className="flex-shrink-0">
                <ProtocolIcon name={icon} className="h-5 w-5 text-slate-300" />
              </div>
            )}
        ...
        <div className="mt-3 flex items-center justify-between border-t border-slate-700/30 pt-2">
          ...
          <div className="text-xs text-slate-500">TVL: ${Math.floor(Math.random() * 1000)}M</div>
        </div>
      </div>
    </div>
  )
